...
async function confirmDelete() {
  ...
  try {
    ...
    const res = await fetch(`http://localhost:8082/api/repair/${props.repair.repairId}`, {
      method: 'DELETE',
      headers: {
  ...
  <div class="modal">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <p>
          你確定要刪除維修單 <strong>#{{ repair.repairId }}</strong> 嗎？
        </p>
        <div class="repair-info">
          <p><strong>機台編號：</strong>{{ repair.machineId }}</p>
          <p><strong>狀態：</strong>{{ repair.repairStatus || repair.status }}</p>
          <p><strong>描述：</strong>{{ repair.repairDescription || repair.description }}</p>
        </div>
